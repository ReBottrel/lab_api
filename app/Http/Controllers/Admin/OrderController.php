...
class OrderController extends Controller
{
    ...
    {
        if (! $request->ajax() || ! $request->filled('animal')) {
            return response()->json(['error' => 'Requisição inválida'], 400);
        }

        $search = trim((string) $request->animal);
        $animals = $this->searchAnimalsByName($search);
     ...
     */
    private function searchAnimalsByName(string $search)
    {
        $base = Animal::with('order')->whereNotNull('order_id');

        if ($search === '') {
            return collect();
        }

        $likeQuery = (clone $base)
            ->where('animal_name', 'LIKE', '%' . $search . '%')
            ->orderBy('animal_name')
            ->limit(50);

        // FULLTEXT exige índice e termos com tamanho mínimo (MySQL ~3 chars)
        if (mb_strlen($search) < 3) {
            return $likeQuery->get();
        }

        $boolean = collect(preg_split('/\s+/u', $search, -1, PREG_SPLIT_NO_EMPTY))
            ->map(function ($word) {
        ...
        try {
            $animals = (clone $base)
                ->whereRaw('MATCH(animal_name) AGAINST(? IN BOOLEAN MODE)', [$boolean])
                ->orderBy('animal_name')
                ->limit(50)
                ->get();
            ...
            // Índice FULLTEXT ausente ou driver sem suporte — usa LIKE
        }
