    ...
    <script>
        ...
        $(document).on('click', '#importar', function() {
            ...
            $.ajax({
                ...
                dataType: 'json',
                beforeSend: function() {
                    $('#importar').html(`<button class="btn btn-primary">IMPORTANDO...</button>`);
                },
                success: function(data) {
                    console.log(data);
                    if (data.success == 'ok') {
                        Swal.fire({
                            title: 'Sucesso!',
                            text: 'Registro importado com sucesso!',
                            icon: 'success',
                    ...
                    if (data.error == 'erro') {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                window.location.href = "{{ route('alelos.get.api') }}";
                            }
                        });
                    }
                    ...
                    if (data.error == 'vazio') {
                        Swal.fire({
                            title: 'Erro!',
                            text: 'Não possui alelos para importar!',
                            icon: 'error',
                            confirmButtonText: 'OK'
                        }).then((result) => {
                            if (result.isConfirmed) {
                ...
                complete: function() {
                    $('#importar').html(`<button class="btn btn-primary">IMPORTAR</button>`);
                }
